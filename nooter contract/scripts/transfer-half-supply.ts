import { ethers } from "hardhat";
import { Contract, Wallet, Provider } from "zksync-ethers";
import { HardhatRuntimeEnvironment } from "hardhat/types";
import dotenv from "dotenv";

dotenv.config();

// Target MultiSwap contract address
const MULTISWAP_ADDRESS = "0x3bbBEC7538e18E45210880CF4866f97f2990Dfa3";

// Token addresses to transfer from
const TOKEN_ADDRESSES = {
  ABSTER: "0xC3f63f74501D225E0CAA6EceA2c8ee73092B3062",
  ABBY: "0x529aF9EbFD8612077bA6b0B72F2898EF7be337D1",
  CHESTER: "0x2460a0068A154C7F2673417dA09f6AE81Ce70e56",
  DOJO3: "0x46BE8d4a214D6ddecE0b3251d76d42E186927781",
  FEATHERS: "0xb4e815813875366e2b4e65eA857278Ae5bEceDc3",
  MOP: "0x45955765a7898f707a523CB1B7a6e3A95DDD5CD7",
  NUTZ: "0x77D29085727405340946919A88B0Ac6c9Ffb80BD",
  PAINGU: "0x8033d82e1e0f949C0986F9102a01C405831b784A",
  PENGUIN: "0x8814046950cDA7aee1B249C1689d070C0db6E58D",
  PUDGY: "0xEcbC4AB2ed8fce5C04dfB1104947Ca4891597336",
  RETSBA: "0x26707CE367C4758F73EF09fA9D8d730869a38e10",
  WOJACT: "0x13D6CbB5f602Df7784bbb9612c5314CDC1ba9d3c",
  YUP: "0xF5048aD4FB452f4E39472d085E29994f6088d96B"
};

// Minimal ERC20 ABI
const ERC20_ABI = [
  "function totalSupply() view returns (uint256)",
  "function transfer(address to, uint256 amount) returns (bool)",
  "function balanceOf(address owner) view returns (uint256)"
];

export default async function (hre: HardhatRuntimeEnvironment) {
  console.log(`Starting token transfer script...`);

  // Ensure the private key is set in the environment variables
  const PRIVATE_KEY = process.env.WALLET_PRIVATE_KEY;
  if (!PRIVATE_KEY) {
    throw new Error("WALLET_PRIVATE_KEY must be set in .env file");
  }

  // Initialize zkSync provider and wallet
  const provider = new Provider(hre.config.networks.zkSyncTestnet.url);
  const wallet = new Wallet(PRIVATE_KEY, provider);

  console.log(`Using wallet address: ${wallet.address}`);
  console.log(`Target MultiSwap address: ${MULTISWAP_ADDRESS}`);

  for (const [tokenName, tokenAddress] of Object.entries(TOKEN_ADDRESSES)) {
    console.log(`\n--- Processing ${tokenName} (${tokenAddress}) ---`);

    try {
      const tokenContract = new Contract(tokenAddress, ERC20_ABI, wallet);

      // Get total supply
      const totalSupply = await tokenContract.totalSupply();
      console.log(`  Total supply: ${ethers.formatUnits(totalSupply, 18)}`);

      // Calculate half supply
      const halfSupply = totalSupply / 2n; // Use BigInt division
      console.log(`  Half supply (amount to transfer): ${ethers.formatUnits(halfSupply, 18)}`);

      // Check deployer's balance
      const deployerBalance = await tokenContract.balanceOf(wallet.address);
      console.log(`  Deployer balance: ${ethers.formatUnits(deployerBalance, 18)}`);

      if (deployerBalance < halfSupply) {
        console.warn(`  ⚠️ Deployer balance is less than half supply for ${tokenName}. Skipping transfer.`);
        continue;
      }

      // Perform the transfer
      console.log(`  Attempting to transfer ${ethers.formatUnits(halfSupply, 18)} ${tokenName} to ${MULTISWAP_ADDRESS}...`);
      const tx = await tokenContract.transfer(MULTISWAP_ADDRESS, halfSupply);

      console.log(`  Transfer transaction sent: ${tx.hash}`);
      await tx.wait(); // Wait for the transaction to be mined
      console.log(`  ✅ Transfer successful for ${tokenName}!`);

      // Verify MultiSwap balance (optional but recommended)
      const multiSwapBalance = await tokenContract.balanceOf(MULTISWAP_ADDRESS);
      console.log(`  MultiSwap balance after transfer: ${ethers.formatUnits(multiSwapBalance, 18)}`);

    } catch (error) {
      console.error(`  ❌ Error processing ${tokenName}:`, error);
    }
  }

  console.log("\nToken transfer script finished.");
} 